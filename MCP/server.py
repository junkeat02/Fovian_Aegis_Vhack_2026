...
def process_agent_command(user_input):
    ...
    model='qwen3:8b',
    messages=[
        {'role': 'system', 'content': 'You are a drone swarm commander. Use tools to move drones and scan for survivors. Always check battery levels.'},
    ...
    if response['message'].get('tool_calls'):
        for call in response['message']['tool_calls']:
            ...
            if func_name == "move_drone":
                return f"EXECUTING: Moving Drone {args['drone_id']} {args['direction']} {args['steps']} steps."

    return response['message']['content']

@app.post("/chat")
async def chat_endpoint(req: ChatRequest):
    # REMOVED 'await' because process_agent_command is a standard function
